...
df1.dataframeName = 'mortality_life_expectancy.csv'
...
df3.dataframeName = 'birth_death_growth_rates.csv'
...
sbd = df1[['code','infant_mortality','year']]
...
fig = go.Figure(data=go.Choropleth(
    ...
    z = (sbdgroup36['youths']/(sbdgroup36['kids']+sbdgroup36['youths']+sbdgroup36['middle_aged']+sbdgroup36['old_aged']))*100,
...
codes = [countries[country] if country != 'I prefer not to say' else None for country in df3['country_name']]

df3['code']=codes



sbd = df3[['code','crude_birth_rate','year']]
sbdgroup36 = sbd[sbd['year']==2036].drop('year',axis=1).groupby('code').mean()
sbdgroup16 = sbd[sbd['year']==2016].drop('year',axis=1).groupby('code').mean()
sbdgroup00 = sbd[sbd['year']==2000].drop('year',axis=1).groupby('code').mean()
sbdgroup90 = sbd[sbd['year']==1990].drop('year',axis=1).groupby('code').mean()
